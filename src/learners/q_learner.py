...
class QLearner:
    ...
    def save_checkpoint_state(self):
        state = {
            "training_steps": self.training_steps,
            "last_target_update_step": self.last_target_update_step,
            "log_stats_t": self.log_stats_t,
        }
        if hasattr(self.target_mac, "agent"):
            state["target_mac"] = self.target_mac.agent.state_dict()
        if self.mixer is not None:
            state["target_mixer"] = self.target_mixer.state_dict()
        if self.args.standardise_returns:
            state["ret_ms"] = self.ret_ms.state_dict()
        if self.args.standardise_rewards:
            state["rew_ms"] = self.rew_ms.state_dict()
        return state

    def load_checkpoint_state(self, state):
        self.training_steps = int(state.get("training_steps", self.training_steps))
        self.last_target_update_step = int(
            state.get("last_target_update_step", self.last_target_update_step)
        )
        self.log_stats_t = int(state.get("log_stats_t", self.log_stats_t))

        target_mac_state = state.get("target_mac")
        if target_mac_state is not None and hasattr(self.target_mac, "agent"):
            self.target_mac.agent.load_state_dict(target_mac_state)
        else:
            self.target_mac.load_state(self.mac)

        if self.mixer is not None:
            target_mixer_state = state.get("target_mixer")
            if target_mixer_state is not None:
                self.target_mixer.load_state_dict(target_mixer_state)
            else:
                self.target_mixer.load_state_dict(self.mixer.state_dict())

        if self.args.standardise_returns and state.get("ret_ms") is not None:
            self.ret_ms.load_state_dict(state["ret_ms"])
        if self.args.standardise_rewards and state.get("rew_ms") is not None:
            self.rew_ms.load_state_dict(state["rew_ms"])
